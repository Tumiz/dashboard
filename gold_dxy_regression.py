...
def generate_predicted_prices(alpha, beta, output="dashboard_monthly.csv"):
    ...
    forecast = round(gold.loc[last_month] * np.exp(alpha), 1)
    out = pd.concat([
        out,
        pd.DataFrame({"pred_price": [forecast]}, index=pd.Index([forecast_month], name="month")),
    ])
    out = out.sort_index().reset_index()
    out = out[["month"] + [c for c in out.columns if c != "month"]]
    out.to_csv(output, index=False)

    print(f"Wrote pred_price for {pred_price.notna().sum()} months")
    print(f"Next-month forecast ({forecast_month}): {forecast} USD")
